...
Object.defineProperty(exports, "__esModule", ({
  ...
  const WHEEL_ZOOM_DISABLED_TIMEOUT = 100; //zoom maximo al abrir el pdf corregir Dominik Dominik
  //dominik estuvo aqui
  ...
  class DefaultExternalServices {
    ...
    static get supportsIntegratedFind() {
      return (0, _pdfjsLib.shadow)(this, "supportsIntegratedFind", false); // dominik false.. poner eb true para desabilitar busqueda en el documento
    }
    ...
    static get supportedMouseWheelZoomModifierKeys() {
      return (0, _pdfjsLib.shadow)(this, "supportedMouseWheelZoomModifierKeys", {
        ctrlKey: true, //Mantiene la Tecla precionada si desactiva el zoom con la ruedita tambien Treste
        metaKey: false  //esta vaina es la tecla windos No sabemos que hace No Tocar!!!
      });
    }
  ...
  const PDFViewerApplication = {
    ...
    pdfRenderingQueue: null,  //DOMINIK null
    ...
    overlayManager: false, //estaba en null dominik dominik   //overlayManager: Gestor de capas superpuestas para diálogos y otros elementos. 
    preferences: null,
    toolbar: null,
    ...
    _touchInfo: null,
    _isCtrlKeyDown: null,    //dominik dominik dominik //false   //al estar en false daba un error con la tecla contorl se cancelaba el wheell
    ...
    // cargue todo el documento en caché y renderice hasta el máximo rango 
      progress(level) {
    ...
    load(pdfDocument) {
      ...
      //de aqui para arriba es El fragmento de código que has proporcionado parece ser parte de la lógica de carga y progreso de PDF.js. Para lograr que PDF.js cargue todo el documento en caché y renderice hasta el máximo rango de zoom, puedes considerar los siguientes pasos:
      ...
      firstPagePromise.then(pdfPage => {
        ...
        Promise.all([_ui_utils.animationStarted, storedPromise, pageLayoutPromise, pageModePromise, openActionPromise]).then(async ([timeStamp, stored, pageLayout, pageMode, openAction]) => {
          ...
          const zoom = _app_options.AppOptions.get("defaultZoomValue");   //zoom dominik variables
    ...
    unbindEvents() {
      ...
      eventBus._off("print",);
      eventBus._off("download", );
      eventBus._off("openinexternalapp",);
  ...
  function webViewerFindFromUrlHash(evt) {
    PDFViewerApplication.eventBus.dispatch("find", {
      ...
      entireWord: false,
      highlightAll: true,
      findPrevious: false,                  // analizar dominik
  ...
  function webViewerWheel(evt) {
    ...
    if (isPinchToZoom || supportedMouseWheelZoomModifierKeys.ctrlKey || evt.metaKey && supportedMouseWheelZoomModifierKeys.metaKey) {
      evt.preventDefault();
  ...
  function webViewerTouchMove(evt) {
    ...
    if (supportsPinchToZoom) {
      ...
      const PIXELS_PER_LINE_SCALE = 330; //doominik valor 30
  ...
  function webViewerKeyDown(evt) {
    PDFViewerApplication._isCtrlKeyDown = evt.key === "Control"; //control dominik dominik
  ...
  const DEFAULT_SCALE_VALUE = "auto"; //"auto"
  ...
  const CursorTool = {     //dominik constante del cursor
    SELECT: 0,
    HAND: 1,
    ZOOM: 2
  };
  ...
  class ProgressBar {
    ...
    setDisableAutoFetch(delay = 10000) {    ///tiempo de precarga de pdf de fabrica 5000 lo coloque en 10seg aver
  ...
  class PDFLinkService {
    ...
    constructor({
      ...
      ignoreDestinationZoom = false  // false dominik
    } = {}) {
      this.eventBus = eventBus;
  ...
  class PDFCursorTools {
    ...
    switchTool(tool) {
      ...
      switch (tool) {
        case _ui_utils.CursorTool.SELECT:      ///aqui dmk dominik handtools
          disableActiveTool();
          break;
        case _ui_utils.CursorTool.HAND:
          disableActiveTool();        //erika
          this._handTool.activate();  //erika
          break;
        case _ui_utils.CursorTool.ZOOM:
        default:
  ...
  class GrabToPan {
    ...
    deactivate() {
      if (this.active) {
        this.active = false;   //dominikk false
        this.element.removeEventListener("mousedown", this._onMouseDown, true);
        this._endPan();
        this.element.classList.remove(CSS_CLASS_GRAB);
      }
    }
    //dmk dominik cursor select 0/1
    toggle() {
      if (this.active) {
        this.deactivate();   //this.deactivate();
      } else {
        this.activate();       //this.activate();  dominik
      }
    }
    ignoreTarget(node) {
    ...
    #onMouseMove(event) {     //movimiento de handtools
  ...
  class PDFDocumentProperties {
    ...
    async #parsePageSize(pageSizeInches, pagesRotation) {
      ...
      let sizeInches = {
        width: Math.round(pageSizeInches.width * 100) / 100,     //dominik *100
        height: Math.round(pageSizeInches.height * 100) / 100    //*100
      };
      let sizeMillimeters = {
        width: Math.round(pageSizeInches.width * 25.4 * 100) / 10,    //* 25.4 * 10   dmk
        height: Math.round(pageSizeInches.height * 25.4 * 100) / 10    // * 25.4 * 10
      };
      let rawName = getPageName(sizeInches, isPortrait, US_PAGE_NAMES) || getPageName(sizeMillimeters, isPortrait, METRIC_PAGE_NAMES);
      if (!rawName && !(Number.isInteger(sizeMillimeters.width) && Number.isInteger(sizeMillimeters.height))) {
        const exactMillimeters = {
          width: pageSizeInches.width * 250.4,      //250.4
          height: pageSizeInches.height * 250.4
        };
        ...
        if (Math.abs(exactMillimeters.width - intMillimeters.width) < 0.1 && Math.abs(exactMillimeters.height - intMillimeters.height) < 0.1) {
          ...
          if (rawName) {
            sizeInches = {
              width: Math.round(intMillimeters.width / 250.4 * 100) / 100,
              height: Math.round(intMillimeters.height / 250.4 * 100) / 100
            };
  ...
  class PDFViewer {
    ...
    #copyCallback(textLayerMode, event) {
      ...
      if (anchorNode && focusNode && selection.containsNode(this.#hiddenCopyElement)) {   //dominik select solo texto no mouse
    ...
    #setScaleUpdatePages(newScale, newValue, {
      ...
      // dominik const postponeDrawing = drawingDelay >= 0 && drawingDelay < 1000;
      this.refresh(true, {
        scale: newScale,
  ...
  class PDFPageView {
    ...
    #textLayerMode = _ui_utils.TextLayerMode.ENABLE; //#textLayerMode: Controla si la capa de texto está habilitada, lo que permite la selección y búsqueda de texto en la página.
    #useThumbnailCanvas = {
      directDrawing: true,
    ...
    update({
      ...
      if (this.canvas) {
        ...
        const postponeDrawing = !onlyCssZoom && drawingDelay >= 0 && drawingDelay < 1000; //1000   dominik
    ...
    async draw() {
      ...
      if (this.maxCanvasPixels === 0) {
        ...
        const maxScale = Math.sqrt(this.maxCanvasPixels / pixelsInViewport); // *2 lo puse yo dominik aumenta la calidad pero baja rendimiento
  ...
  class SecondaryToolbar {
    ...
    #bindCursorToolsListener({    // cursosr handtools dominik
  ...
  class BasePreferences {
    #defaults = Object.freeze({
      ...
      "annotationMode": 2,
      "cursorToolOnLoad": 0,
      "defaultZoomDelay": 0, //dominik 400 // efecto zoom
      "defaultZoomValue": "auto",   //auto para que al abrir no alla ese conflicto de demasiado zoom dominik 
      "disablePageLabels": true,  // false //disablePageLabels: Si se desactivan las etiquetas de página.
      "enablePermissions": false,
      "enablePrintAutoRotate": true,
      "enableScripting": false,   //true //enableScripting: Si se permite la ejecución de scripts dentro del PDF.
      ...
      "pdfBugEnabled": false,      //modo desarrollo false  se quita true habilita
      "sidebarViewOnLoad": -1,
      "scrollModeOnLoad": -1,  //-1 dominik
      "spreadModeOnLoad": -1,   //-1
      "textLayerMode": 1,
      "viewerCssTheme": 0,
      "viewOnLoad": 0,
      "disableAutoFetch": false, //disableAutoFetch: Si se desactiva la precarga automática de páginas.
  ...
  //abajo dominik render page veamos si podemos subir 
    ...
    return Promise.all([pdfDocument.getPage(pageNumber), printAnnotationStoragePromise]).then(function ([pdfPage, printAnnotationStorage]) {
      const renderContext = {
        ...
        viewport: pdfPage.getViewport({
          scale: 1,
          //scale: 1 DominikCell subir la escala no aunmenta la imagen y puede generar problema en pantallas pequeñas
  ...
  class PDFPrintService {
    ...
    layout() {
      ...
      if (!hasEqualPageSizes) {
        console.warn("No todas las páginas tienen el mismo tamaño. ¡El resultado impreso puede ser incorrecto!");
      }
    ...
    throwIfInactive() {
      if (!this.active) {
        throw new Error("Esta solicitud de impresión fue cancelada o completada.");
      }
    }
  }
  ...
  window.print = function () {
    if (activeService) {
      console.warn("Se ignoró window.print() debido a un trabajo de impresión pendiente");
      return;
    }
    ensureOverlay().then(function () {
      if (activeService) {
  ...
  function renderProgress(index, total, l10n) {
    ...
    const progress = Math.round(1000 * index / total);  100
  ...
  function getXfaHtmlForPrinting(printContainer, pdfDocument) {
    ...
    const scale = Math.round(_pdfjsLib.PixelsPerInch.PDF_TO_CSS_UNITS * 10000) / 1000;
