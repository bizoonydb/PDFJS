...
document.getElementById('iframeContainer').addEventListener('contextmenu', function(event) {
  ...
  if (iframe) {
    if (!iframe.dataset.cursorToggled || iframe.dataset.cursorToggled === "false") {
      ...
      // guarda timers de descarregar iframes
...
function switchTab(tabId) {
  ...
  // mostra o iframe ativo
  const activeIframe = iframes.get(tabId);
  activeIframe.classList.remove('hidden');

  // se estava descarregado, recarrega
  if (activeIframe.src === 'about:blank') {
    activeIframe.src = activeIframe.dataset.originalUrl;
  }

  // se havia timer para esse iframe, cancela (pois voltou a ser ativo)
  if (unloadTimers.has(tabId)) {
    clearTimeout(unloadTimers.get(tabId));
    unloadTimers.delete(tabId);
  }
