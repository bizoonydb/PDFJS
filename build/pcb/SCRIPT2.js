...
//////////////////////////
 ...
 let blinkInterval = 30;  // Intervalo em frames para alternar as cores (ajuste conforme necessário)
...
function mousePressed() {
 ...
 function drawComponentBoundingBoxes() {
    ...
    for (let part of parts) {
        ...
        if (part.pins.length > 0) {
            ...
            if (part.name.startsWith("C")) {
                fillColor = color(255, 255, 150, 85); // Vermelho translúcido para "CAPACITOR"
            } else if (part.name.startsWith("R")) {
                fillColor = color(50, 50, 50, 100); // Verde translúcido para "RRESISTOR"
            } else if (part.name.startsWith("MIC")) {
                fillColor = color(255, 255, 150, 225); // Verde translúcido para "RRESISTOR"
            } else if (part.name.startsWith("N")) {
                fillColor = color(0, 0, 255, 0); // Azul translúcido para "NETNAME"
            } else if (part.name.startsWith("L")) {
                fillColor = color(30, 30, 30, 205); // Azul translúcido para "BOBINA"
            } else if (part.name.startsWith("ZD")) {
                fillColor = color(80, 80, 80, 125); // Azul translúcido para "DIODO ZAINER"
            } else if (part.name.startsWith("U")) {
                fillColor = color(50, 50, 50, 100); // Azul translúcido para "C-I"
            } else if (part.name.startsWith("S")) {
                fillColor = color(128, 128, 128, 115); // Azul translúcido para "SIMCARD"
            } else if (part.name.startsWith("H")) {
                fillColor = color(50, 50, 50, 120); // Azul translúcido para "FPC"
            } else if (part.name.startsWith("J")) {
                fillColor = color(200, 200, 200, 115); // Azul translúcido para "CONECTOR"
            } else if (part.name.startsWith("D")) {
                fillColor = color(80, 80, 80, 125); // Azul translúcido para "DIODO"
            } else if (part.name.startsWith("Q")) {
                fillColor = color(80, 80, 80, 125); // Azul translúcido para "TRANSISTOR"
            } else if (part.name.startsWith("RF")) {
                fillColor = color(255, 255, 0, 105); // Azul translúcido para "ANTENA"
            } else if (part.name.startsWith("ANT")) {
                fillColor = color(128, 128, 128, 555); // Azul translúcido para "ANTENA"
            } else if (part.name.startsWith("PA")) {
                fillColor = color(50, 50, 50, 120); // Azul translúcido para "CI TAMBEM"
            } else {
                fillColor = color(255, 255, 0, 0); // Amarelo translúcido para outros
            }
            ...
            backgroundRects.push({ x: minX + groupOffset, y: minY, w: maxX - minX, h: maxY - minY, color: fillColor });
             ...
             // Desenha os pinos e bordas por cima
            stroke(255, 255, 255);
    ...
    //  1. Desenhamos os fundos PRIMEIRO
    noStroke();
