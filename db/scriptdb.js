        ...
        // Definir tema no carregamento da página
...
// Função de troca de tema
        function setTheme(theme, save = true) {
            ...
            document.querySelectorAll('.theme-btn').forEach(btn => {
                ...
                if (btn.textContent.includes(theme.charAt(0).toUpperCase() + theme.slice(1))) {
                    btn.classList.add('active-theme');
                    btn.innerHTML = `<i class="fas fa-check"></i> ${theme.charAt(0).toUpperCase() + theme.slice(1)}`;
                } else {
                    //Redefinir outros botões para seus ícones originais
                    if (btn.classList.contains('theme-blue-btn')) {
                        btn.innerHTML = '<i class="fas fa-palette"></i> Blue';
                    } else if (btn.classList.contains('theme-green-btn')) {
                        btn.innerHTML = '<i class="fas fa-leaf"></i> Green';
                    } else if (btn.classList.contains('theme-red-btn')) {
                        btn.innerHTML = '<i class="fas fa-fire"></i> Red';
                    } else if (btn.classList.contains('theme-white-btn')) {
                        btn.innerHTML = '<i class="fas fa-snowflake"></i> White';
                    }
                }
            });

           
            document.documentElement.style.setProperty('--main-color', themeColors[theme] || '#00ccff');
        }
...
function openTab(fileUrl, type) {
  ...
  if (type === 'pdf') {
    iframe.src = `https://bizoonydb.github.io/PDFJS/web/aa.html?file=${encodeURIComponent(fileUrl)}`;
  } else if (type === 'bvr') {
  ...
  iframe.dataset.originalUrl = fileUrl;
  iframe.dataset.fileType = type;
  iframe.setAttribute('sandbox', 'allow-scripts allow-same-origin');
iframe.setAttribute('scrolling', 'no'); // Oculta rolagem na maioria dos navegadores
...
// Adiciona funcionalidade de contexto no iframe
document.getElementById('iframeContainer').addEventListener('contextmenu', function(event) {
  ...
  if (iframe) {
    if (!iframe.dataset.cursorToggled || iframe.dataset.cursorToggled === "false") {
      iframe.dataset.cursorToggled = "true";
      ...
      iframe.dataset.cursorToggled = "false";
