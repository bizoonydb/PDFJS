...
function draw() {
    background(255); // Pone el fondo blanco en cada frame.
    ...
    if (isLoading) { // Si está cargando, muestra mensaje de carga animado.
        ...
        text("MODO 2D", width / 2, height / 2); // Texto "Cargando" centrado.
...
function drawOutline(points, offsetX = 0, flipX = false, mirrorAxis = 0) {
    ...
    drawingContext.shadowColor = 'rgba(0, 0, 0, 20)';  // sombra preta semi-transparente
    drawingContext.shadowBlur = 15;                      // desfoque da sombra
    drawingContext.shadowOffsetX = 8;                     // deslocamento X
    drawingContext.shadowOffsetY = 8;                     // deslocamento Y

    stroke('BLACK');      // cor da borda
    strokeWeight(5);
    fill(0, 255, 0, 130); // cor da placa
    ...
    // Desliga a sombra para não afetar desenhos futuros
...
drawingContext.shadowBlur = 8;  // desfoque da sombra proporcional ao zoom
